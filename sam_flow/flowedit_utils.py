...
def FlowEditFLUX(pipe,
    ...
    packed_h = x_src.shape[2] // 2
    packed_w = x_src.shape[3] // 2
    latent_image_ids = torch.zeros(packed_h, packed_w, 3)
    latent_image_ids[..., 1] = latent_image_ids[..., 1] + torch.arange(packed_h)[:, None]
    latent_image_ids[..., 2] = latent_image_ids[..., 2] + torch.arange(packed_w)[None, :]
    latent_src_image_ids = latent_image_ids.reshape(-1, 3).to(device=device, dtype=x_src_packed.dtype)
