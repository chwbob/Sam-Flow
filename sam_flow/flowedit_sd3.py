...
def load_sd3_pipeline(config: Dict, device: str | None = None, dtype: torch.dtype | None = None) -> StableDiffusion3Pipeline:
    ...
    model_id = config["models"]["sd3_model_id"]

    pipe = StableDiffusion3Pipeline.from_pretrained(model_id, torch_dtype=requested_dtype)
    pipe = pipe.to(requested_device)
    pipe.set_progress_bar_config(disable=True)
    return pipe


def generate_scout_image_sd3(
    ...
    with torch.no_grad():
        ...
        x_src = x_src * pipe.vae.config.scaling_factor

        out_latents = FlowEditSD3(
            pipe=pipe,
        ...
        latents_dec = out_latents / pipe.vae.config.scaling_factor
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate SD3 scout images with FlowEdit.")
    ...
    for case in cases:
        ...
        print(f"[flowedit_sd3] saved scout for {case['image_name']} / {case['target_code']} -> {scout_path}")
